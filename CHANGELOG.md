...
- **Screenplay projects.** New Project asks Novel or Screenplay; a screenplay is one script written in Fountain syntax and saved as a `.fountain` file, edited all at once. See `docs/screenplay-plan.md`.
  - **Scenes.** The Chapters sidebar becomes a Scenes list built from the scene headings, the chapter shortcuts jump between scenes and reorder them, and renaming a scene row rewrites its heading. A screenplay project holds one script, and the chapter keys work on its scenes: Delete Chapter takes the scene the caret is in (or every scene a selection touches) out of the script and into Trash as a document of its own, Restore puts a trashed scene back at the end of the script for the reorder keys to carry up, and moving the last scene down stashes it at the top of Reference, from where moving it up brings it back. A stashed or trashed scene edits as a script but moves, renames and trashes as one document however many scenes it grows to.
  - **Writing.** Enter and Tab move between screenplay elements (Ctrl+1 to Ctrl+6 open a new element, Ctrl+Alt+1 to Ctrl+Alt+6 change the current line's type, Shift+Enter makes a line break inside an element, Ctrl+Enter starts a scene heading, Ctrl+Shift+Enter opens a picker of every element, Ctrl+D marks dual dialogue). Typing `INT. ` on an action line makes it a heading; headings, cues and transitions are kept in capitals as they are typed; a cue gets `(CONT'D)` when the same character speaks again after action; and Tab after a heading's place puts in the ` - ` for the time. The foot of the Project Notes panel carries a Format block naming the element of the line the caret is on - Scene Heading, Action, Dialogue - which shows and hides with the rest of that panel.
  - **Suggestions.** Character names, locations, scene intros, times of day, transitions and cue extensions complete as they are typed, an empty cue suggests the next speaker, Enter on a suggested name goes straight to the speech, and the suggestions can be switched off in Settings. Nothing has to be refreshed for a name just typed into the script to be offered.
  - **Characters/Locations**, under Tools on a screenplay project's menu only, shows the two lists the cues and headings complete from, with the number of lines of the script each name is written into. Add a character or a location the script has not reached yet, so it completes from the first line typed for it. Rename a character and every use of the name in the script is rewritten - their cues, and the action, dialogue, parentheticals and scene headings they are named in, each in the case that line wrote it, so DAN becomes BEN and Dan becomes Ben. A lower-case spelling is left alone outside their cues, since a name is also a word (will, mark, rose), and the rename is described before it is made: how many cues and other lines it reaches, and a warning that a word spelled like the name may be replaced by mistake where a sentence begins with it ("Will you come?" for a character called WILL). A place is renamed in its scene headings only. Either way, Ctrl+Z takes the whole rename back as one step. Remove a name and it stops being offered while the script keeps every line it is in - for the one-off DAN in scene one who answers every attempt at DANIELLE; a removed name keeps its row, marked, and Restore puts it back. Refresh takes a list back to what the script itself holds.
  - **Page Count.** Word Count becomes a Page Count for a script: the page estimate, the session's change in pages, and a goal in pages with the progress bar, and none of the word figures. The estimate counts only what a script prints: a section, a synopsis, a note or a boneyard takes no page space, so the figure does not grow with the notes a writer leaves in the script. The editor draws a dotted rule with the page number where each page is estimated to turn (a Settings switch). The Outliner breaks a screenplay down by scene: a row per scene heading, the pages that scene is estimated to fill (in eighths, as in Page Count) and its synopsis line as the summary.
  - **Title page.** Properties edits the title page.
  - **Import and export.** Export for a screenplay is a Save As of one file as PDF, Final Draft `.fdx`, `.fountain` or plain text. A screenplay project's Import dialog offers only the script formats - `.fountain`, Final Draft `.fdx` and Fade In `.fadein`, Fountain first - plus plain text, which comes in as a Reference document; the docx, HTML and EPUB options, chapter splitting and the chapter label choice are left out. A script imported into the project takes the script's place, with its title page, and the script it replaces goes to Trash as one block. A novel project's dialog leaves the script formats out, the same way round: a script imported into a novel came in as a chapter the editor held in screenplay mode, in a project whose sidebar, word count and export treat every chapter as prose.
  - **Menus and Reference.** In a screenplay project the menus grey out what is not for a script (Compile, the chapter tools while the script is showing, the manuscript conversions) and the Word Count item reads Page Count. Reference documents stay prose, and Add New Chapter (Ctrl+N) makes one while the script is showing.
