...
test('insertChapTitle inserts a centered header title when none exists', async function(){
...
test('insertChapTitle replaces the text of an existing header title', async function(){
...
test('insertChapTitle preserves the blank first line instead of swallowing it when titling', async function(){
...
test('insertChapTitle leaves an unloadable chapter untouched', async function(){
...
test('renumberChaps sets sequential titles from the template', async function(){
  var chapA = makeChapter(untitledFirstLine());
  var chapB = makeChapter(untitledFirstLine());
  var project = makeProject([chapA, chapB]);

  await renumberChaps(project, 0, 1, false, true, 'Chapter [num]');

  assert.strictEqual(chapA.title, 'Chapter 1');
  assert.strictEqual(chapB.title, 'Chapter 2');
});

test('renumberChaps uses number words when useNumerals is false', async function(){
  var chap = makeChapter(untitledFirstLine());
  var project = makeProject([chap]);

  await renumberChaps(project, 0, 0, false, false, 'Chapter [num]');

  assert.strictEqual(chap.title, 'Chapter One');
});

test('renumberChaps only rewrites chapter contents when withinChaps is true', async function(){
  var chap = makeChapter(untitledFirstLine());
  var originalContents = chap.contents;
  var project = makeProject([chap]);

  await renumberChaps(project, 0, 0, false, true, 'Chapter [num]');

  assert.strictEqual(chap.contents, originalContents);
  assert.strictEqual(chap.hasUnsavedChanges, true);
});

test('renumberChaps rewrites chapter contents with the new title when withinChaps is true', async function(){
  var chap = makeChapter(untitledFirstLine());
  var project = makeProject([chap]);

  await renumberChaps(project, 0, 0, true, true, 'Chapter [num]');

  assert.strictEqual(chap.contents.ops[0].insert, 'Chapter 1');
  assert.strictEqual(project.hasUnsavedChanges, true);
});

test('renumberChaps falls back to a numeral past the last spelled-out number word', async function(){
  var chaps = [];
  for(let i = 0; i < 101; i++)
    chaps.push(makeChapter(untitledFirstLine()));
  var project = makeProject(chaps);

  await renumberChaps(project, 0, 100, false, false, 'Chapter [num]');

  assert.strictEqual(chaps[99].title, 'Chapter One Hundred');
  assert.strictEqual(chaps[100].title, 'Chapter 101');
});

test('renumberChaps leaves the project unmarked when the range is empty', async function(){
  var chap = makeChapter(untitledFirstLine());
  var project = makeProject([chap]);
  var originalTitle = chap.title;

  await renumberChaps(project, 1, 0, false, true, 'Chapter [num]');

  assert.strictEqual(chap.title, originalTitle);
  assert.strictEqual(project.hasUnsavedChanges, false);
});

test('renumberChaps clamps an out-of-range endIndex to the last chapter', async function(){
  var chapA = makeChapter(untitledFirstLine());
  var chapB = makeChapter(untitledFirstLine());
  var project = makeProject([chapA, chapB]);

  await renumberChaps(project, 0, 5, false, true, 'Chapter [num]');
